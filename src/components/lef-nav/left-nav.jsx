...
class LeftNav extends Component {
    ...
    hasAuth=(item)=>{
        ...
        const userMessage = this.props.user  // 从redux中获取state-user
        const {username} = userMessage
        const {menus} = userMessage.role  // 获取menus权限
        /* 如下情况返回true，反则为false
                ...
                if(!item.children) {
                    return ( /* antd 格式 */
                        <Menu.Item className="lef-nav-menu-item" key={item.key} icon={item.icon}>
                            <Link to={item.key} onClick={()=>{this.props.setHeadTitle(item.title)}}>{item.title}</Link>  {/* 点击导航栏，发送更新title的action */}
                        </Menu.Item>
                    )
                } else {
...
export default connect(
    ...
    {setHeadTitle}
)(withRouter(LeftNav)) // withRouter为高阶组件，给非路由组件传递3个props属性
